...
import {
    saveSnapshot, getSnapshots, getTeamConfig, saveTeamConfig,
    getRiskThreshold, saveRiskThreshold, getScanExtensions, saveScanExtensions
    ...
    panel.webview.onDidReceiveMessage(async (msg) => {
        switch (msg.command) {
            case 'refresh':
                ...
                break;
            case 'saveSettings':
                if (msg.threshold !== undefined) { saveRiskThreshold(Number(msg.threshold)); }
                if (msg.teamConfig !== undefined) { saveTeamConfig(msg.teamConfig); }
                if (msg.extensions !== undefined) { saveScanExtensions(msg.extensions); }
...
async function exportCSV(blocks: any[]) {
    ...
    const rows = blocks.map((b: AIBlock) =>
        `${b.employeeId},${b.date},"${b.file}",${b.lines},"${b.editedBy.join(' | ')}",${b.hash || ''}`
    ).join('\n');
...
function generateHTML(
    ...
    const fileTotalLines: Record<string, number> = {};
...
<div class="tab-panel" id="tab-trends">
  ...
  <div class="chart-card">
    ...
    <table style="width:100%">
      ...
      <tbody>
      ${[...getSnapshots()].reverse().slice(0, 20).map(s => `
        <tr>
          <td>${s.date}</td>
...
const snapshots  = ${JSON.stringify(getSnapshots())};
